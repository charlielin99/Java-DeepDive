...
public class Solution {
    ...
    public static int[] getProductsOfAllIntsExceptAtIndex(int[] intArray) {
        ...
        if (intArray.length < 2){
            throw new IllegalArgumentException("Cannot be done!");
        }
        
        int[] productArrayExceptIndex = new int[intArray.length];
        int productArrayMultiplier = 1;
        
        for (int i=0; i<intArray.length; i++){
            productArrayExceptIndex[i] = productArrayMultiplier;
            productArrayMultiplier *= intArray[i];
        }
        
        productArrayMultiplier = 1;
        for (int i=intArray.length-1; i>=0; i--){
            productArrayExceptIndex[i] *= productArrayMultiplier;
            productArrayMultiplier *= intArray[i];
        }

        return productArrayExceptIndex;
    }
