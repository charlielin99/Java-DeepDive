...
public class Solution {
    ...
    public static int[] mergeArrays(int[] myArray, int[] alicesArray) {
        ...
        int[] totalArray = new int[myArray.length + alicesArray.length];
        
        int currentTotalIndex = 0;
        int currentMyIndex = 0;
        int currentAlicesIndex = 0;
        
        while (currentTotalIndex < totalArray.length){
            boolean myArrayExhausted = currentMyIndex >= myArray.length;
            boolean alicesArrayExhausted = currentAlicesIndex >= alicesArray.length;
            
            if (!myArrayExhausted && (alicesArrayExhausted || myArray[currentMyIndex] < alicesArray[currentAlicesIndex])){
                totalArray[currentTotalIndex] = myArray[currentMyIndex];
                currentMyIndex++;
            } else {
                totalArray[currentTotalIndex] = alicesArray[currentAlicesIndex];
                currentAlicesIndex++;
            }
