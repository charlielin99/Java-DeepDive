...
public class Solution {
    ...
    public static int changePossibilities(int amount, int[] denominations) {
        int[] waysOfDoingCents = new int[amount+1];
        waysOfDoingCents[0] =1;
        
        for (int coin: denominations){
            for (int higherAmount = coin; higherAmount <= amount; higherAmount++){
                int higherAmountRemainder = higherAmount - coin;
                waysOfDoingCents[higherAmount] += waysOfDoingCents[higherAmountRemainder];
            }
        }
        
        return waysOfDoingCents[amount];
    }
