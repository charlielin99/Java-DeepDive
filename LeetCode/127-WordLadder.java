...
The idea is that reached always contain only the ones we just reached in the last step, and wordDict always contain the ones that haven't been reached. BFS always finds shortest path.
since this is a bfs solution, it will always check the possible result before it goes deeper and gurantee shortest path

Time complexity for one directional graph traversal = (n26)^d
Space complexity for one directional graph traversal = (n26)^d/
n is the number of words and d is length of the word
    */
